...
class Oppsett:
    ...
    def sett_pos_slaggull(slagh):
        ...
        rotasjon = Oppsett.rotasjonsgrad_spillebrett(forsvarer._spiller)
    ...
    def sett_pos_slagstyrke(slagh,spiller):
        ...
        rotasjon = Oppsett.rotasjonsgrad_spillebrett(spiller._spiller)
        rotasjon_tur = (90 if rotasjon == 270 else (270 if rotasjon == 90 else rotasjon))
        ...
        rotp_x,rotp_y = rotasjonspunkt[kortstilling][rotasjon_tur]
        ...
        for i,spillekort in enumerate(styrkekort):
            ...
            kort_x, kort_y = Oppsett.roter_objekt(sorkort_x,sorstyrke_y,KB,KH,rot_punkt,rotasjon_tur,spillekort)
    ...
    def sett_pos_spillekort_kastebunke(kastebunke,spkort):
        ...
        offsett_x = random.randint(-2,2)
        offsett_y = random.randint(-2,2)

        antall = len(kastebunke)
        
        kort_x = x-(antall-1)//8

        spkort.sett_rotasjon(spkort._rotasjon + (spkort._rotasjon - 90) % 180)
        spkort.sett_gyldig_pos((kort_x+offsett_x,y+offsett_y))

        if not spkort._animeres:
            spkort.sett_pos((kort_x+offsett_x,y+offsett_y))
    ...
    def sett_pos_deltakerhand(deltaker):
        ...
        rotasjon = Oppsett.rotasjonsgrad_hånd(spiller)
    ...
    @staticmethod
    def rotasjonsgrad_spillebrett(spiller):
        if spiller == "Sør": return 0
        if spiller == "Vest": return 270
        if spiller == "Nord": return 180
        if spiller == "Øst": return 90
        raise ValueError(f"Value {spiller} not valid.")
    
    @staticmethod
    def rotasjonsgrad_hånd(spiller):
        if spiller == "Sør": return 0
        if spiller == "Vest": return 90
        if spiller == "Nord": return 0
        if spiller == "Øst": return 270
        raise ValueError(f"Value {spiller} not valid.")

...
class Spillvindu:
    ...
    def tegn_scoreboard(self):
        ...
        rect_surf = pygame.Surface((b, h), pygame.SRCALPHA)
        rect_color = (255, 255, 255, 128)
        pygame.draw.rect(rect_surf, rect_color, rect_surf.get_rect())
        self._brett.blit(rect_surf, (x, y))
        
        self.tegn_tekstboks(f"SPILLET ER OVER", SVART, "Arial", 30, (x+50, y+50))

        vinnere = self._spill.vinnere()
        if len(vinnere) >= 3:
            vinnermelding = ', '.join(map(lambda x: x._spiller, vinnere[:-1])) + " og " + vinnere[-1]._spiller + " vant!"
        elif len(vinnere) == 2:
            vinnermelding = vinnere[0]._spiller + " og " + vinnere[1]._spiller + " vant!"
        else:
            vinnermelding = vinnere[0]._spiller + " vant!"
        self.tegn_tekstboks(vinnermelding, SVART, "Arial", 20, (x+50, y+100))

        for i,deltaker in enumerate(self._spill._deltakere.values()):
            self.tegn_tekstboks(f"{deltaker._spiller}: {deltaker.antall_gull()}p", SVART, "Arial", 18, (x+50, y+150+i*20))
    ...
    def fyll_brett(self,figurer):
        ...
        for deltaker in self._spill._deltakere.values():
            if deltaker.er_ferdig():
                self.tegn_tekstboks(deltaker._spiller, ROD, "Arial", 14, deltaker._tekstboks)
                continue
    ...
    def spillekjoring(self):
        ...
        while running:
            ...
            if self._spill.er_spilleforers_tur() and self._spill._spilleforer.er_ferdig():
            ...
            for event in pygame.event.get(): 
                ...
                if event.type == pygame.MOUSEBUTTONDOWN:
                    ...
                    if self.marker_passes(musepeker):
                        if not har_spilt and self._spill._trekkbunke.er_tom():
                            self._spill._spilleforer.sett_til_ferdig()
    ...
    def marker_spillbar(self, musepeker):
        ...
        if self._spill._spilleforer.er_ferdig():
            return None
